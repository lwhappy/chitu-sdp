...
object HadoopUtils extends Logger {
  ...
  private[this] var flinkHttpURL: String = _
  ...
  private[this] def getFileSystem(hadoopConf: Configuration): FileSystem = {
    Try(FileSystem.get(hadoopConf)) match {
      case Success(fs) => fs
      case Failure(e) =>
        throw new IllegalArgumentException(s"[SDP] access hdfs error.$e")
    }
  }
      ...
      synchronized {
        if (rmHttpURL == null || getLatest) {
          ...
          val name = if (!HAUtil.isHAEnabled(conf)) addressPrefix else {
            ...
            require(activeRMId != null, "[SDP] can not found yarn active node")
  ...
  def getFlinkWebAppURL(): String = {
    if (flinkHttpURL == null) {
      synchronized {
        if (flinkHttpURL == null) {
          val conf = hadoopConf
          val useHttps = YarnConfiguration.useHttps(conf)
          val flinkAddress = conf.get("yarn.web-proxy.address")
          if (useHttps) {
            flinkHttpURL = "https://" + flinkAddress
          } else {
            flinkHttpURL = "http://" + flinkAddress
          }
        }
      }
    }
  ...
  def getFlinkWebAppURL(hadoopConfDir: String): String = {
    val conf = initHadoopConfNew(hadoopConfDir)
    val useHttps = YarnConfiguration.useHttps(conf)
    val flinkAddress = conf.get("yarn.web-proxy.address")
    if (useHttps) {
      "https://" + flinkAddress
  ...
  @throws[IOException] def downloadJar(jarOnHdfs: String, rootDir: String): String = {
    val tmpDir = FileUtils.createTempDir(rootDir)
  ...
  def initHadoopConfNew(hadoopConfDir: String): Configuration = {
    val conf = getConfigurationFromHadoopConfDir(hadoopConfDir)
    //add hadoopConfDir to classpath...you know why???
    ClassLoaderUtils.loadResource(hadoopConfDir)
  ...
  def getRMWebAppURL(getLatest: Boolean, hadoopConfDir: String): String = {
    if (rmHttpURL == null || getLatest) {
      synchronized {
        if (rmHttpURL == null || getLatest) {
