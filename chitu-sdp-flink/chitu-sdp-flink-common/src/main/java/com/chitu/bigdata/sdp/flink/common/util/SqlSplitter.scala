...
object SqlSplitter {
   ...
   * @return
   */
  private[flink] def extractLineBreaks(text: String) = {
  ...
  private[flink] def isSingleLineComment(text: String) = text.trim.startsWith("--")

  private[flink] def isMultipleLineComment(text: String) = text.trim.startsWith("/*") && text.trim.endsWith("*/")
   ...
   * @return
   */
  private[flink] def isSingleLineComment(curChar: Char, nextChar: Char): Boolean = {
