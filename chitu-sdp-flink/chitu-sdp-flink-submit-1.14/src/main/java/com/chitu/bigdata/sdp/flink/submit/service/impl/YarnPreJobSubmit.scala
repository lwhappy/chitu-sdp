  ...
  override def doSubmit(submitRequest: SubmitRequest): SubmitResponse = {
    ...
    val clusterDescriptor = {
      ...
      val flinkDistJar = new File(s"${submitRequest.flinkHome}/lib").list().filter(_.matches("flink-dist_.*\\.jar")) match {
        case Array() => throw new IllegalArgumentException(s"[SDP] can no found flink-dist jar in ${submitRequest.flinkHome}/lib")
        case array if array.length == 1 => s"${submitRequest.flinkHome}/lib/${array.head}"
        case more => throw new IllegalArgumentException(s"[SDP] found multiple flink-dist jar in ${submitRequest.flinkHome}/lib,[${more.mkString(",")}]")
      }
