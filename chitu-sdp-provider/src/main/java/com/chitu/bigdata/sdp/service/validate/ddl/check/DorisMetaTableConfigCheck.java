...
@Component
@Slf4j
public class DorisMetaTableConfigCheck extends AbstractMetaTableConfigCheck {
    ...
    public SqlExplainResult check(SdpMetaTableConfig sdpMetaTableConfig, SdpDataSource sdpDataSource) {
        try {
            ...
            return  checkCreateTableName(sqlNodes,"table-name",sdpMetaTableConfig.getMetaTableName(),"表单中的表名和ddl属性中的table-name名称不一致");
        } catch (Exception e) {
           log.error("解析sql出错",e);
        }
        return null;
